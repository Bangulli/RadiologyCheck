...
def infer(ds, out):
    ...
    for sample in tqdm.tqdm(ds, desc='Inferring'):
        specialty = to_english(sample['specialty']) if needs_to_english(sample['specialty']) else sample['specialty']
        ## User prompt ----------------------------------------------------------------------------
        prompt = {
                "role": "user",
                "content": [
        ...
        ## save output
        os.makedirs(out/sample['id'], exist_ok=True)
        with open(out/sample['id']/'generated_report.txt', 'w') as f:
            f.write(result)

def infer_eval(ds, out):
    out = pl.Path(out)
    os.makedirs(out, exist_ok=True)
    with open("config.json", "r") as f:
        cfg = json.load(f)
    ...
    for sample in tqdm.tqdm(ds):
        ...
        except: print(f'Inference error in {sample["id"]}, skipping.')
        ...
        ## run inference
        try: result = run_medgemma(messages, pth=cfg['inf'])
        except: print(f'Inference error in {sample["id"]}, skipping.')
        ...
        bleus[sample['id']] = {"bleu": bleu_score, "SemSim": semsim}
    
    with open(out/'eval.json', 'w') as f:
        json.dump(bleus, f, indent=4)
